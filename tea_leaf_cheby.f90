...
SUBROUTINE tea_leaf_cheby_init(level, theta)
...
!$OMP DO
    DO t=1,tiles_per_task
      CALL tea_leaf_kernel_cheby_init(chunk(level)%tiles(t)%field%x_min,&
...
SUBROUTINE tea_leaf_cheby_iterate(level, ch_alphas, ch_betas, max_cheby_iters, cheby_calc_steps)
...
!$OMP DO
    DO t=1,tiles_per_task
      CALL tea_leaf_kernel_cheby_iterate(chunk(level)%tiles(t)%field%x_min,&
                  chunk(level)%tiles(t)%field%x_max,                       &
                  chunk(level)%tiles(t)%field%y_min,                       &
                  chunk(level)%tiles(t)%field%y_max,                       &
                  halo_exchange_depth,                       &
